use deepthought::{DeepThoughtBuilder, DeepThoughtRouterBuilder};
...
fn main() {
    let mut dtr = DeepThoughtRouterBuilder::default()
        ...
        .build()
        .unwrap();
    let dtb = DeepThoughtBuilder::new()
        .dbpath("./main_db".to_string())
    ...
    dtr.new_route("default", dtb).unwrap();
    let model = dtr.get_route("default").unwrap();
    let _ = model.register_template("default", TEMPLATE).unwrap();
    let _ = model.add_document_with_sync(&DOCUMENT);
    let res = dtr
        .query("default", "Who is Anna Karenina?", "default")
        .unwrap();
    println!("Returned from model:");
    println!("{}", &res.get("chat").unwrap().cast_string().unwrap());
    println!("Returned from RAG");
    for d in res.get("rag").unwrap().cast_list().unwrap() {
        println!("{}", d.cast_string().unwrap());
        println!(">>>>>>>>>>>>");
    }
}
